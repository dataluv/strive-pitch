...
export default function StriveAgent() {
  ...
  const sendMessage = useCallback((text: string) => {
    ...
    if (lowerText.includes("fluid") || lowerText.includes("bolus")) {
      ...
      response = `[Risk Stratification] Patient ${patient.name}\n\nMortality Risk: ${patient.riskScore}%\nSOFA Score: ${patient.sofa}/24\nAcuity: ${patient.acuity.toUpperCase()}\n\n${critLabs.length > 0 ? `Critical values: ${critLabs.map(l => `${l.name} ${l.value} ${l.unit}`).join(", ")}` : "No critical lab values."}\n\nComorbidities: ${patient.comorbidities.join(", ")}\n\nThe RL model predicts ${patient.acuity === "improving" ? "continued improvement" : patient.acuity === "critical" ? "high risk -- aggressive management needed" : "moderate trajectory with optimization potential"} over the next 48 hours if the recommended treatment policy is followed.`;
    } else if (lowerText.includes("sofa") || lowerText.includes("organ")) {
      response = `[SOFA Analysis] Total SOFA: ${patient.sofa}/24\n\n${patient.sofaComponents.map(c => `${c.name}: ${c.score}/4 (${c.detail})`).join("\n")}\n\nSOFA trend is ${patient.acuity === "improving" ? "improving" : patient.acuity === "critical" ? "worsening" : "stable"}. Each 1-point increase in SOFA is associated with approximately 3-5% increase in ICU mortality based on our cohort analysis.`;
    } else if (lowerText.includes("antibiotic") || lowerText.includes("abx") || lowerText.includes("drug")) {
      response = `[Antibiotic Review] Source: ${patient.sepsisSource}\n\nCurrent regimen:\n${patient.antibioticRegimen.map(a => `- ${a.name} ${a.dose} ${a.route} ${a.frequency} (started ${a.started})`).join("\n")}\n\n${patient.creatinine > 1.5 ? `NOTE: Creatinine ${patient.creatinine} mg/dL -- renal dose adjustment may be required.` : "Renal function adequate for current dosing."}\n\nAllergies: ${patient.allergies.join(", ")}`;
    } else if (lowerText.includes("compare") || lowerText.includes("similar") || lowerText.includes("cohort")) {
      const sameSource = patients.filter(p => p.sepsisSource === patient.sepsisSource);
      response = `[Cohort Comparison] Patient ${patient.name} vs. similar patients:\n\n${sameSource.length} patients with ${patient.sepsisSource}:\n- Avg Risk: ${Math.round(sameSource.reduce((s, p) => s + p.riskScore, 0) / sameSource.length)}%\n- Avg SOFA: ${Math.round(sameSource.reduce((s, p) => s + p.sofa, 0) / sameSource.length * 10) / 10}\n\nThis patient's risk (${patient.riskScore}%) is ${patient.riskScore > Math.round(sameSource.reduce((s, p) => s + p.riskScore, 0) / sameSource.length) ? "above" : "below"} the cohort average.`;
    } else if (lowerText.includes("hipaa") || lowerText.includes("compliance") || lowerText.includes("security") || lowerText.includes("on-premise") || lowerText.includes("llm") || lowerText.includes("openai") || lowerText.includes("claude")) {
      response = `[Compliance & Architecture]\n\nStrive is fundamentally different from LLM-based tools (ChatGPT, Claude, etc.):\n\n1. ON-PREMISE DEPLOYMENT: All patient data stays within the hospital's infrastructure. No data leaves the building.\n\n2. NOT AN LLM: Our core model is a reinforcement learning agent trained on 5M+ hours of de-identified ICU data. It computes optimal actions, not text.\n\n3. HIPAA COMPLIANT: Full BAA coverage, SOC2 Type II certified, HITRUST ready.\n\n4. WHY HOSPITALS CAN'T USE OPENAI/CLAUDE: Sending PHI to cloud providers violates HIPAA. Even with BAAs, the risk profile is unacceptable for real-time clinical decisions. Strive's on-premise RL model eliminates this entirely.\n\n5. MOAT: Our model is trained on proprietary ICU outcome data. It's not a fine-tuned LLM -- it's a purpose-built RL system that can't be replicated by prompting GPT-4.`;
    } else {
      response = `[RL Clinical Agent] For patient ${patient.name} (${patient.age}y ${patient.sex}, ${patient.sepsisSource}, SOFA ${patient.sofa}, Risk ${patient.riskScore}%):\n\n${patient.recommendation.rationale}\n\nKey insights:\n${patient.agentInsights.slice(0, 3).map(i => `- ${i}`).join("\n")}\n\nThis analysis is powered by our reinforcement learning model trained on 5M+ ICU hours. Ask me about fluid management, vasopressors, risk stratification, cohort comparisons, or drug interactions for more specific analysis.`;
    }
        ...
        <div className="flex-1 flex flex-col bg-gray-50">
          ...
          <div className="bg-white border-t border-gray-200 p-4 shrink-0">
            <div className="max-w-3xl mx-auto">
              <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2 focus-within:border-[#00B894] focus-within:ring-2 focus-within:ring-[#00B894]/20 bg-white">
                {/* File upload */}
                <button onClick={() => alert("File upload — attach patient records, imaging reports, or lab results. HIPAA-compliant processing on-premise.")} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Upload file">
                ...
                {/* Image upload */}
                <button onClick={() => alert("Image upload — attach X-rays, CT scans, or clinical photos. All processing happens on-premise. HIPAA compliant.")} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Upload image">
                ...
                />
                {/* Voice */}
                <button onClick={() => alert("Voice input — speak your clinical question. Transcription and RL analysis happen entirely on-premise. No data leaves the hospital network.")} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Voice input">
                ...
                {/* Video / Camera */}
                <button onClick={() => alert("Camera — activate bedside camera for real-time visual assessment. Video stream processed locally with on-premise computer vision. HIPAA compliant.")} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Camera / Video">
                ...
                <button
                  ...
                  className="p-2.5 bg-[#00B894] text-white rounded-lg hover:bg-[#00a383] transition-colors disabled:opacity-50"
                >
